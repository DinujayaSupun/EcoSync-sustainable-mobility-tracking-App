      ...
      if (req.user && req.user._id.toString() === id) {
        ...
        if (role.toLowerCase() !== user.role) {
          return res.status(403).json({
            ...
            errors: [{ 
              field: "role", 
              message: "You cannot change your own admin role. Ask another administrator to do this." 
            }]
          });
        }
      }
  ...
  try {
    ...
    if (req.user && req.user._id.toString() === id) {
      return res.status(403).json({
        ...
        errors: [{ 
          field: "id", 
          message: "You cannot delete your own account. This is a security measure." 
        }]
      });
    }
    ...
    if (user.role === 'admin') {
      ...
      if (adminCount <= 1) {
        return res.status(403).json({
          ...
          errors: [{ 
            field: "id", 
            message: "This is the last admin account. Create another admin before deleting this one." 
          }]
        });
      }
    }
...
exports.getActivityLogs = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      logs: [],
      pagination: { currentPage: 1, totalPages: 0, totalLogs: 0, logsPerPage: 50 }
    });
  }
};
