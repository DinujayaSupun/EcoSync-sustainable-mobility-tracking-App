...
const calculateHaversineDistance = (startCoords, destCoords) => {
...
const calculateRoute = async (startCoords, destCoords, transportType) => {
  try {
    ...
    console.warn(`⚠️ OSRM API failed (${error.message}), using fallback distance calculation`);
    // Fallback to Haversine distance calculation
    return calculateHaversineDistance(startCoords, destCoords);
  }
};
  ...
  try {
    ...
    if (!startLocation || !destination || !transportType) {
      return res.status(400).json({
        success: false,
        message: "Please provide start location, destination, and transport type",
      });
    }
