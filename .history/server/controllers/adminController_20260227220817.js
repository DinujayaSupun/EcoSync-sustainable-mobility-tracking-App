  ...
  try {
    ...
    if (!name && !email && !faculty && !role) {
      return res.status(400).json({
        ...
        errors: [{ field: "body", message: "At least one field (name, email, faculty, role) must be provided" }]
      });
    }
    ...
    if (email && email !== user.email) {
      ...
      if (emailExists) {
        return res.status(400).json({
          ...
          errors: [{ field: "email", message: `Email '${email}' is already registered to another user` }]
        });
      }
    }
    ...
    if (name !== undefined) {
      ...
      if (name.trim().length > 100) {
        return res.status(400).json({
          success: false,
          message: "Invalid name",
          errors: [{ field: "name", message: "Name must not exceed 100 characters" }]
        });
      }
    }
    ...
    if (faculty !== undefined) {
      if (typeof faculty !== 'string' || faculty.trim().length < 2) {
        return res.status(400).json({
          ...
          errors: [{ field: "faculty", message: "Faculty must be at least 2 characters long" }]
        });
      }
    }
    ...
    if (role !== undefined) {
      if (!["user", "admin"].includes(role.toLowerCase())) {
        return res.status(400).json({
          ...
          errors: [{ field: "role", message: "Role must be either 'user' or 'admin'" }]
        });
      }
    ...
    console.log(`[ADMIN UPDATE] User ${id} updated by admin. Changes:`, {
      name: originalValues.name !== user.name ? { from: originalValues.name, to: user.name } : 'unchanged',
      email: originalValues.email !== user.email ? { from: originalValues.email, to: user.email } : 'unchanged',
      faculty: originalValues.faculty !== user.faculty ? { from: originalValues.faculty, to: user.faculty } : 'unchanged',
      role: originalValues.role !== user.role ? { from: originalValues.role, to: user.role } : 'unchanged',
    });
    ...
    res.status(500).json({
      ...
      errors: [{ field: "server", message: "An unexpected error occurred. Please try again later." }]
    });
  }
};
