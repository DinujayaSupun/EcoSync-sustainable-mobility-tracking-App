...
Keep it professional and suitable for presentation to university administrators.`;
    ...
    if (error.response) {
      ...
      if (error.response.status === 429) {
        userMessage = 'OpenAI API quota exceeded. Please check your billing at https://platform.openai.com/account/billing or contact your administrator.';
      } else if (error.response.status === 401) {
        userMessage = 'OpenAI API key is invalid or expired. Please update your API key.';
      }
    }
    ...
    res.status(500).json({
      ...
      fallbackMessage: 'AI insights are temporarily unavailable. Please review the data summary manually.'
    });
  }
};