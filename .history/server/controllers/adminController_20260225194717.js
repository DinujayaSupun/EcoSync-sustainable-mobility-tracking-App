...
// adminController.js - Add new function
exports.emailReport = async (req, res) => {
  try {
    ...
    const msg = {
      to: req.user.email,
      from: 'admin@ecosync.com',
      subject: 'Weekly Sustainability Report',
      html: `
        <h2>EcoSync Weekly Report</h2>
        <p>Total CO2 Saved: ${reportData.totalCO2} kg</p>
        <p>Active Users: ${reportData.activeUsers}</p>
        <p>Total Trips: ${reportData.totalTrips}</p>
      `
    };
...
exports.notifySlack = async (message) => {
...
// Use it in existing functions
exports.getAdminStats = async (req, res) => {
  // ... existing code ...
  
  // Send daily summary to Slack
  if (stats.totalCO2 > 1000) {
    await notifySlack(`🎉 Milestone! Total CO2 saved: ${stats.totalCO2}kg`);
  }
