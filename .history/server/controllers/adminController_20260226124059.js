...
Keep it professional and suitable for presentation to university administrators.`;
    ...
    if (error.message?.includes('API_KEY_INVALID')) {
      userMessage = 'Gemini API key is invalid. Please get a valid key at https://aistudio.google.com/app/apikey';
    } else if (error.message?.includes('QUOTA_EXCEEDED')) {
      userMessage = 'Gemini API quota exceeded. Please try again later or check your usage.';
    } else if (error.status === 429 || error.message?.includes('429')) {
      userMessage = 'API rate limit exceeded. Please try again in a few moments.';
    } else if (error.message?.includes('quota')) {
      userMessage = 'API quota exceeded. The free tier has generous limits - please try again later.';
    }
