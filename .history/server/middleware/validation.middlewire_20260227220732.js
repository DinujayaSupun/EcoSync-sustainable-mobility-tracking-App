...
const validateUserUpdate = [
  ...
  body("faculty")
    ...
    .matches(/^[a-zA-Z\s&-]+$/)
    .withMessage("Faculty can only contain letters, spaces, hyphens, and ampersands"),
...
const validateAtLeastOneField = (req, res, next) => {
  ...
  if (!name && !email && !faculty && !role) {
    return res.status(400).json({
      success: false,
      message: "At least one field (name, email, faculty, or role) must be provided for update",
      errors: [{
        field: "body",
